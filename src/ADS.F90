...
module ADSS
   ...
   use time_scheme, ONLY: ConfigureBackwardEuler3D, ConfigureDouglasGunn, &
                          ConfigureDouglasGunn3D, ConfigurePeacemanRachford3D
...
!> scheme out of \ref Step, prepares the 3D Douglas-Gunn coefficient
!> tables with \ref ConfigureDouglasGunn3D, and delegates the actual
!> three directional iGRM solves to \ref MultiStep.
!>
...
!>
!> @details
!> The wrapper keeps the Peaceman-Rachford scheme on the \ref MultiStep
!> path. It prepares the 3D PR coefficient tables and executes the three
!> directional substeps. iGRM space compatibility is expected to be
!> checked once during problem setup by \ref ValidateIGRMTimeSchemeSpaces.
!
! Input:
! ------
!> @param[in] iter
...
!
!---------------------------------------------------------------------------
subroutine PeacemanRachfordStep(iter, RHS_fun, ads_test, ads_trial, ads_data, n, mierr, RHS_point, &
                              include_transport)
      use Setup, ONLY: ADS_Setup, ADS_compute_data
      use Interfaces, ONLY: forcing_fun, rhs_point_fun
      implicit none
...
end subroutine PeacemanRachfordStep
...
!>
!> @details
!> The wrapper keeps Backward Euler on the \ref MultiStep path. It
!> prepares directional Backward Euler coefficient tables and executes the
!> three directional substeps. iGRM space compatibility is expected to be
!> checked once during problem setup by \ref ValidateIGRMTimeSchemeSpaces.
!
! Input:
! ------
...
!
!---------------------------------------------------------------------------
subroutine BackwardEulerStep(iter, RHS_fun, ads_test, ads_trial, ads_data, n, mierr, RHS_point, &
                           include_transport)
      use Setup, ONLY: ADS_Setup, ADS_compute_data
      use Interfaces, ONLY: forcing_fun, rhs_point_fun
      implicit none
...
end subroutine BackwardEulerStep
