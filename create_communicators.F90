...
integer :: processors_Z(NRPROCZ)
  ...
  ! get the integer coordinates of the processor in the 3D box
  ...
  if (iprint == 1) then
    do i = 1,NRPROCX
      do j = 1,NRPROCY
        do k = 1,NRPROCZ
          write(*,*)PRINTRANK,'processors(',i,j,k,')=',processors(i,j,k)
        enddo
      enddo
    enddo
  endif
  do i = 1,NRPROCX
    do j = 1,NRPROCY
      ...
      call mpi_group_incl(group_comm_world,NRPROCZ,processors_Z, GROUPZ(i,j),i2)
      if (i2 /=0 ) then
  ...
  do i = 1,NRPROCX
    do k = 1,NRPROCZ
      ...
      call mpi_group_incl(group_comm_world,NRPROCY,processors_Y,GROUPY(i,k),i2)
      if (i2 /= 0) then
  ...
  do j = 1,NRPROCY
    do k = 1,NRPROCZ
      ...
      call mpi_group_incl(group_comm_world,NRPROCX,processors_X,GROUPX(j,k),i2)
      if (i2 /= 0) then
  ...
  do i = 1,NRPROCX
    do j = 1,NRPROCY
      call mpi_comm_create(MPI_COMM_WORLD,GROUPZ(i,j),comm_myrank_local,i3)
  ...
  do i = 1,NRPROCX
    do k = 1,NRPROCZ
      call mpi_comm_create(MPI_COMM_WORLD,GROUPY(i,k),comm_myrank_local,i3)
  ...
  do j = 1,NRPROCY
    do k = 1,NRPROCZ
      call mpi_comm_create(MPI_COMM_WORLD,GROUPX(j,k),comm_myrank_local,i3)
