...
- iGRM pollution executes two real physical source steps, validates the zero
  ...
  grid;
- oil uses the opt-in `ADS_OIL_RANDOM_SEED` test seed, requires the accumulated
  drained result to increase from one to two physical steps, and compares the
  two-step result across one/four OpenMP threads and a hybrid MPI run.
